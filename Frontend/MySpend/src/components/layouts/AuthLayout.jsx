      ...
      <header className="w-full bg-[#f5f3f2] shadow-sm sticky top-0 z-50"> 
        <div className="w-full px-6 md:px-12 py-4 flex items-center">
          ...
          <div className="ml-auto flex items-center gap-5">
            ...
            <Link to="/signup">
              <button className="px-4 py-2 rounded-md bg-violet-600 text-white text-sm hover:bg-blue-700 cursor-pointer">
                Sign up
              </button>
            </Link>
          </div>
  ...
  <div className="mx-auto w-screen max-w-screen-xl px-4 py-16 sm:px-6 sm:py-24 lg:px-8 lg:py-32">
    <div className="mx-auto max-w-prose text-center">
      ...
      <p className="mt-4 text-base text-pretty text-gray-900 sm:text-lg/relaxed">
        Start tracking your expenses today with <strong className="font-bold">MySpend</strong> and save tons of money!
      </p>

      <div className="mt-4 flex justify-center gap-4 sm:mt-6">
        <a
          className="inline-block rounded border bg-violet-600 border-primary bg-primary px-5 py-3 font-medium text-white shadow-sm transition-colors hover:bg-indigo-700"
          href="#"
        >
          Get Started
        </a>
...
const StatsInfoCard = ({ icon, label, value, color }) => {
  return (
    <div className="flex gap-4 items-center bg-white p-4 rounded-xl shadow-md border border-gray-200">
      <div className={`w-12 h-12 flex items-center justify-center text-[26px] bg-violet-600 text-white ${color} rounded-full`}>
        {icon}
      </div>
      <div>
