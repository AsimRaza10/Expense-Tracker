...
export const CustomBarChartForLast30DaysExpense = (data = []) => {
  const sortedData = [...data].sort((a, b) => new Date(a.date) - new Date(b.date))

  const chartData = sortedData.map((item) => ({
    date: moment(item?.date).format("Do MMM"),  // add date
    amount: item?.amount,
    category: item?.category, // keep if you still want tooltip/category use
  }))
